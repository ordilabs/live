...
pub fn ThemeToggle(cx: Scope) -> impl IntoView {
    ...
    view! { cx,
        <ActionForm action=set_dark_action >
            <input
                type="hidden"
                // By convention `name` has to be as same as server function `SetDarkTheme` expects its parameters
                // In this case `is_dark`
                name="is_dark"
                value=move || (!is_dark()).to_string()
            />
            <button
                type="submit"
                ...
                inner_html=move || {
                  if is_dark() {
                    r#"<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" class="w-5 h-5">
